...
static void
update_r(kmeans_baseline_config *config)
{
	int i;
	...
	for (i = 0; i < config->num_objs; i++)
	{
		...
		*/
		if (!obj)
		{
			config->clusters[i] = kmeans_baseline_NULL_CLUSTER;
			continue;
		}
...
static void
update_means(kmeans_baseline_config *config)
{
	int i;
...
kmeans_baseline(kmeans_baseline_config *config)
{
	int iterations = 0;
	int *clusters_last;
	...
	if (!config->max_iterations)
		config->max_iterations = kmeans_baseline_MAX_ITERATIONS;

	/*
	 * Previous cluster state array. At this time, r doesn't mean anything
	 * but it's ok
	 */
	clusters_last = kmeans_baseline_malloc(clusters_sz);
	...
	while (1)
	{
		...
		{
			kmeans_baseline_free(clusters_last);
			config->total_iterations = iterations;
			return kmeans_baseline_OK;
		}
		...
		if (iterations++ > config->max_iterations)
		{
			kmeans_baseline_free(clusters_last);
			config->total_iterations = iterations;
			return kmeans_baseline_EXCEEDED_MAX_ITERATIONS;
		}
	}

	kmeans_baseline_free(clusters_last);
	config->total_iterations = iterations;
	return kmeans_baseline_ERROR;
}
